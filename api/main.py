...
def train_xgboost(X_train, X_valid, y_train, y_valid, params_dict):
    ...
    imp_dict = {
        k: float(v)
        for k, v in sorted(imp.items(), key=lambda x: x[1], reverse=True)[:15]
    }
...
def train_logistic_regression(
    ...
    coef = (
        pd.Series(lr.coef_.ravel(), index=feature_names)
        .abs()
...
async def train_model(
    ...
    try:
        ...
        if "label" not in df.columns:
            raise HTTPException(
                status_code=400, detail="CSV must contain 'label' column"
            )
...
async def predict(request: PredictRequest):
    ...
    try:
        ...
        if model_type == "xgboost":
            ...
            X_imp = pd.DataFrame(model["imputer"].transform(X), columns=feature_names)
        ...
        return PredictResponse(
            predictions=probabilities, predicted_labels=predicted_labels
        )
...
async def list_models():
    ...
    return {"models": list(all_models), "loaded_in_memory": list(models.keys())}
    ...
    try:
        ...
        if model_type == "xgboost":
            ...
            if not file.filename.endswith(".zip"):
                raise HTTPException(
                    status_code=400,
                    detail="XGBoost models must be uploaded as .zip files",
                )
            ...
            if not file.filename.endswith(".pkl"):
                raise HTTPException(
                    status_code=400,
                    detail="Random Forest and Logistic Regression models must be .pkl files",
                )
