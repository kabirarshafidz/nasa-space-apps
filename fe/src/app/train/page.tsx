...
// Force dynamic rendering to avoid hydration mismatch with search params
...
export default function TrainPage() {
    ...
    const loadTrainingEntry = async () => {
      ...
      try {
        const response = await fetch(
          `/api/training/entries?sessionId=${selectedSessionId}`
        );
        const data = await response.json();

        if (data.success && data.entries.length > 0) {
          const entry = data.entries.find(
            (e: TrainingEntryData) => e.id === entryId
          );
  ...
  const handleStartTraining = async (sessionId: string, hasCSV: boolean) => {
    ...
    if (hasCSV) {
      try {
        const response = await fetch(
          `/api/training/entries?sessionId=${sessionId}`
        );
        const data = await response.json();
  ...
  const handleTrain = async () => {
    ...
    if (!file && !usingExistingCSV) {
      setTrainingError(
        "No CSV file provided. Please upload a file or use an existing session with data."
      );
      return;
    }
  ...
  const handleFineTuneSubmit = async () => {
    ...
    if (!file && selectedSessionId) {
      ...
      if (sessionResult.success && sessionResult.session?.csvUrl) {
        ...
        setTrainingError(
          "Session doesn't have a CSV file. Please upload a new file."
        );
        return;
      }
    }
              ...
              <CardContent className="pt-6">
                ...
                {currentStep === 1 && (
                  <UploadDataStep
                    ...
                    maxSize={maxSize}
                    handleDragEnter={
                      handleDragEnter as (e: React.DragEvent<Element>) => void
                    }
                    handleDragLeave={
                      handleDragLeave as (e: React.DragEvent<Element>) => void
                    }
                    handleDragOver={
                      handleDragOver as (e: React.DragEvent<Element>) => void
                    }
                    handleDrop={
                      handleDrop as (e: React.DragEvent<Element>) => void
                    }
                    ...
                    {usingExistingCSV && (
                      <div className="mb-4 p-4 bg-primary/10 border border-primary/30 rounded-lg">
                        <p className="text-sm text-primary-foreground">
                          ℹ️ Using existing dataset from this session. The CSV
                          will be automatically included in training.
                        </p>
                      </div>
                    )}
                    <ConfigureModelStep
          ...
          <DialogContent className="sm:max-w-5xl bg-background border-primary/30 max-w-4xl max-h-[90vh] flex flex-col">
            <DialogHeader>
              ...
              <DialogDescription className="text-primary-foreground/70">
                Adjust all training parameters and retrain the model with the
                same dataset.
              </DialogDescription>
            </DialogHeader>
