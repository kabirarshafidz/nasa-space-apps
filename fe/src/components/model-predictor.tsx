...
export default function ModelPredictor() {
  ...
  const [singlePrediction, setSinglePrediction] =
  ...
  const loadModels = async () => {
    ...
    try {
      const response = await fetch(
        "https://sm1kbhd2-8000.aue.devtunnels.ms/models"
      );
  ...
  const predictRow = async (rowIndex: number) => {
    ...
    try {
      ...
      const response = await fetch(
        "https://sm1kbhd2-8000.aue.devtunnels.ms/predict",
        {
          method: "POST",
                            ...
                            <Popover>
                              <PopoverTrigger asChild>
                                <Button
                                  size="sm"
                                  onClick={() =>
                                    !row._prediction && predictRow(actualIndex)
                                  }
                                  disabled={row._isLoading || !selectedModel}
                                  variant={
                                    row._prediction
                                      ? row._prediction.label === 1
                                        ? "default"
                                        : "secondary"
                                      : "outline"
                                  }
                                ...
                                <PopoverContent className="w-80">
                                  <div className="space-y-3">
                                    <div className="space-y-2">
                                      ...
                                      <p className="text-sm text-muted-foreground">
                                        Classification details for this data
                                        point
                                      </p>
                                    </div>
                                    <div className="space-y-2">
                                      ...
                                      <div className="flex items-center justify-between">
                                        ...
                                        <span className="text-sm">
                                          {(
                                            row._prediction.probability * 100
                                          ).toFixed(2)}
                                          %
                                        </span>
                                      </div>
                                      <div className="flex items-center justify-between">
                                        ...
                                        <span className="text-sm font-mono">
                                          {row._prediction.probability.toFixed(
                                            6
                                          )}
                                        </span>
                                      </div>
            ...
            <div className="p-6 rounded-lg border bg-card space-y-4">
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between p-4 rounded-lg bg-muted">
                  ...
                  <Badge
                    variant={
                      singlePrediction.label === 1 ? "default" : "secondary"
                    }
                    className="text-base px-4 py-1"
                  >
                    {singlePrediction.label === 1
                      ? "Exoplanet Detected"
                      : "No Exoplanet"}
                  </Badge>
                </div>
